									...
									<div class="tab-pane active" id="vtab${status.index+1}">
									   ...
									   <!--  <form class="form-horizontal" role="form"> -->
										...
										<c:if test="${fn:length(ups) > 0}">
                                           ...
                                           <div class="space"></div>
										<!-- <i class="icon-check pr-10">Lorem:</i>  ipsum dolor sit amet maiores -->
												<c:if test="${prototype.pt_entity==1}">
												 <li>  <i class="icon-check pr-10">有无样品实体:</i>有样机</li>
												</c:if>
												<c:if test="${prototype.pt_entity==0}">
												  <li>  <i class="icon-check pr-10">有无样品实体:</i>无样机，有设计图</li>
												</c:if>
												
												<c:if test="${prototype.pt_market==1}">
												   <li> <i class="icon-check pr-10">市场选择策略:</i>密集性市场策略</li>
												</c:if>
												<c:if test="${prototype.pt_market==2}">
												   <li> <i class="icon-check pr-10">市场选择策略:</i>产品专业化策略</li>
												</c:if>
												<c:if test="${prototype.pt_market==3}">
												   <li> <i class="icon-check pr-10">市场选择策略:</i>市场专业化策略</li>
												</c:if>
												<c:if test="${prototype.pt_market==4}">
												   <li> <i class="icon-check pr-10">市场选择策略:</i>有选择的专业化策略</li>
												</c:if>
												<c:if test="${prototype.pt_market==5}">
												   <li> <i class="icon-check pr-10">市场选择策略:</i>全部市场策略</li>
												</c:if>
												
												<c:if test="${prototype.pt_product==1}">
												   <li> <i class="icon-check pr-10">将来的产品定位:</i>奢侈品</li>
												</c:if>
												<c:if test="${prototype.pt_product==2}">
												   <li> <i class="icon-check pr-10">将来的产品定位:</i>高端产品</li>
												</c:if>
												<c:if test="${prototype.pt_product==3}">
												   <li> <i class="icon-check pr-10">将来的产品定位:</i>行业针对性产品</li>
												</c:if>
												<c:if test="${prototype.pt_product==4}">
												   <li> <i class="icon-check pr-10">将来的产品定位:</i>必需品</li>
												</c:if>
												
												<li> <i class="icon-check pr-10">预期价格:</i>${prototype.pt_price}</li>
												
												<c:if test="${prototype.pt_safety==1}">
												   <li> <i class="icon-check pr-10">安全属性:</i>对安全属性要求最高</li>
												</c:if>
												<c:if test="${prototype.pt_safety==2}">
												   <li> <i class="icon-check pr-10">安全属性:</i>对安全属性要求较高</li>
												</c:if>
												<c:if test="${prototype.pt_safety==3}">
												   <li> <i class="icon-check pr-10">安全属性:</i>不属于安全性要求高的领域</li>
												</c:if>
												
												<c:if test="${prototype.pt_compete==1}">
												   <li> <i class="icon-check pr-10">市场竞争性:</i>所属领域为垄断性</li>
												</c:if>
												<c:if test="${prototype.pt_compete==2}">
												   <li> <i class="icon-check pr-10">市场竞争性:</i>同类型产品竞争激烈</li>
												</c:if>
												<c:if test="${prototype.pt_compete==3}">
												   <li> <i class="icon-check pr-10">市场竞争性:</i>同类型产品竞争不激烈</li>
												</c:if>
												
												<c:if test="${prototype.pt_ditch==1}">
												   <li> <i class="icon-check pr-10">渠道特性:</i>销售渠道集中</li>
												</c:if>
												<c:if test="${prototype.pt_ditch==2}">
												   <li> <i class="icon-check pr-10">渠道特性:</i>销售渠道分散</li>
												</c:if>
												<!-- 写到这里 -->
												
												<c:if test="${prototype.pt_aimgroup==1}">
												   <li> <i class="icon-check pr-10">目标群体:</i>专业人员</li>
												</c:if>
												<c:if test="${prototype.pt_aimgroup==2}">
												   <li> <i class="icon-check pr-10">目标群体:</i>高端人士</li>
												</c:if>
												<c:if test="${prototype.pt_aimgroup==3}">
												   <li> <i class="icon-check pr-10">目标群体:</i>普通大众</li>
												</c:if>
												<c:if test="${prototype.pt_aimgroup==4}">
												   <li> <i class="icon-check pr-10">目标群体:</i>中小企业</li>
												</c:if>
												<c:if test="${prototype.pt_aimgroup==5}">
												   <li> <i class="icon-check pr-10">目标群体:</i>大型企业</li>
												</c:if>
										</div> --%>
										 ...
										 <div style="text-align: center">
										  <c:forEach items="${inNexts}" var="inNext">
											   ...
											   <c:if test="${inNext.in_parent == in.in_id}">
											     ...
											     <li><i class="icon-check pr-10"></i> ${inNext.in_id}</li>
											     <li><i class="icon-check pr-10"></i> ${inNext.in_name}</li>
