				...
				<li style="width:800px">
					<article class="entry-item box-hover clearfix">
						<table>
							<tr>
								<div style="width:700px">										
									<c:if test="${fn:length(row.title) > 30 }">
										<td>
											<a style="font-size: 20px;" href="csNewsDetail?id=${row.id }">
							...
							<tr>
								<div>
									<c:if test="${fn:length(row.newsContent) > 160 }">
										<td>
											${fn:substring(row.newsContent,0,160)}...
										</td>
									</c:if>
									<c:if test="${fn:length(row.newsContent) <= 160 }">
										<td>
					...
					<div class="dataTables_paginate paging_bs_full" id="datatable1_paginate" style="height:50px">
						...
						<ul class="pagination" style="padding-left: 0;margin: 20px 0;display: inline-block;">
							<li ${pager.pageNum==1?'class="disabled"':''}><a
								href="chinaSecurityNews?pageNum=1">首页</a></li>
							<li ${pager.pageNum==1?'class="disabled"':''}><a
								href="chinaSecurityNews?pageNum=${pager.pageNum-1 }">上页</a></li>
							<li ${pager.pageNum==pager.pageCount?'class="disabled"':''}><a
								href="chinaSecurityNews?pageNum=${pager.pageNum+1 }">下页</a></li>
							<li ${pager.pageNum==pager.pageCount?'class="disabled"':''}><a
								href="chinaSecurityNews?pageNum=${pager.pageCount }">末页</a></li>
							<li><a>转到 </a></li>
