			...
			<div class="box border purple">
					...
					<div class="box-body">
						<table id="datatable1" cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered table-hover">
							...
							<tbody>
								<c:forEach items="${performancePreview}" var="v" varStatus="sta">
								<tr class="gradeX">
								
								    <td><a href="company?stockNum=${fn:substring(v.stockCode,0,6)}">${v.stockName}</a></td>
									<td>${v.previewType }</td>
									<td>${v.previewAbstract }</td>
									<td>${v.previewAmplitude }</td>
									<td>${v.netProfitUplimit }</td>
									<td>${v.netProfitDownlimit }</td>
									<td>${v.previewDate }</td>
								</tr>
								</c:forEach>
							</tbody>
