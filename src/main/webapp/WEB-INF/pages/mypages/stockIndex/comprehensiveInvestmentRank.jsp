			...
			<div class="box border purple">
					...
					<div class="box-body">
						<table id="datatable1" cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered table-hover">
							...
							<tbody>
								<c:forEach items="${comprehensiveInvestmentRank}" var="v" varStatus="sta">
								<tr class="gradeX">
								
								    <td><a href="company?stockNum=${fn:substring(v.stockCode,0,6)}">${v.stockName}</a></td>
									<td>${v.sixRankChar }</td>
									<td>${v.sixRankNum }</td>
									<td>${v.institutionCount }</td>
									<td>${v.upInstitutionCount }</td>
									<td>${v.downInstitutionCount }</td>
									<td>${v.holdInstitutionCount }</td>
									<td>${v.sixBuy }</td>
									<td>${v.sixIncreace }</td>
									<td>${v.sixNeuter }</td>
									<td>${v.sixDecreace }</td>
