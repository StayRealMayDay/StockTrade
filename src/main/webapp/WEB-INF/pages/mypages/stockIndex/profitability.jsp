						...
						<table id="datatable1" cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered table-hover">
							<thead>
								<tr>
									...
									<th>总资产报酬率ROA（TTM）</th>
									<th>总资产净利率ROA（TTM）</th>
									<th>投入资本回报率ROIC（TTM）</th>
									<th>销售净利率（TTM）</th>
									<th>销售毛利率（TTM）</th>
									<th>净利润／营业总收入（TTM）</th>
									<th>营业利润／营业总收入（TTM）</th>
									<th>营业总成本／营业总收入（TTM）</th>
									<th>销售费用／营业总收入（TTM）</th>
									<th>管理费用／营业总收入（TTM）</th>
									<th>财务费用／营业总收入（TTM）</th>
									<th>资产减值损失／营业总收入（TTM）</th>
								</tr>
							</thead>
	
							<tbody>
								<c:forEach items="${profitability}" var="v" varStatus="sta">
								<tr class="gradeX">
									<td><a href="company?stockNum=${fn:substring(v.stockCode,0,6)}">${v.stockName}</a></td>
									<td>${v.netAssetYieldroeTtm }</td>
									<td>${v.totalAssetRewardRateroaTtm }</td>
									<td>${v.totalAssetNetRateroaTtm }</td>
									<td>${v.returnCapitalroicTtm }</td>
									<td>${v.netSalesRateTtm }</td>
									<td>${v.grossMarginSalesTtm }</td>
									<td>${v.netProfitTotalIncomeTtm }</td>
									<td>${v.oprateProfitTotalIncomeTtm }</td>
									<td>${v.totalCostTotalIncomeTtm }</td>
									<td>${v.salesTotalIncomeTtm }</td>
									<td>${v.managementTotalIncomeTtm }</td>
									<td>${v.finacialTotalIncomeTtm }</td>
									<td>${v.assetLossTotalIncomeTtm }</td>
								</tr>
