...
.secondary {border:1px solid #CCCCCC;height:800px;margin-left:20px;margin-top:40px;}
</style>
						...
						<div id="filter-controls" class="btn-group"  style="margin-bottom: 2%">
							<form class="form-inline">
								...
								<div class="col-md-12 agileinfo_news_original_grids_left">
									...
									<c:forEach items="${stockAllList}" var="all">
										<tr>
											<td></td>
											<td><a href="company">${all.stockName}</a></td>
											<td><a href="company">${all.stockNum}</a></td>
											<td>${all.currentPrice}</td>
											<td>${all.updownRatio }</td>
											<td>${all.updownPrice }</td>
											<td>${all.upSpeed }</td>
											<td>${all.turnoverRate }</td>
											<td>${all.equivalentRatio }</td>
											<td>${all.amplitude}</td>
											<td>${all.turnVolume}</td>
											<td>${all.floatingStock}</td>
											<td>${all.circulationMarketValue}</td>
											<td>${all.peRatio}</td>
											<td></td>
										</tr>
									</c:forEach>	
									...
									<div class="dataTables_paginate paging_bs_full"
										...
										<ul class="pagination">
											...
											<li ${pager.pageNum==1?'class="disabled"':''}><a
												href="marketIndex?pageNum=1">首页</a></li>
											<li ${pager.pageNum==1?'class="disabled"':''}><a
												href="marketIndex?pageNum=${pager.pageNum-1 }">上页</a></li>
											<li ${pager.pageNum==pager.pageCount?'class="disabled"':''}><a
												href="marketIndex?pageNum=${pager.pageNum+1 }">下页</a></li>
											<li ${pager.pageNum==pager.pageCount?'class="disabled"':''}><a
												href="marketIndex?pageNum=${pager.pageCount }">末页</a></li>
											<li><a>转到 </a></li>
