...
<div class="wrapper">
	<div id="main-content">
		...
		<ul id="myTab1" class="col-md-12 nav nav-tabs" role="tablist" >					
			<c:forEach items="${newStockList}" var="row">	
				<li style="width:800px">
					<article class="entry-item box-hover clearfix">
						<table>
							<tr>
								<div style="width:700px">										
									<c:if test="${fn:length(row.newsTittle) > 30 }">
										<td>
											<a style="font-size: 18px;" href="AstockNewsDetail?id=${row.ddzzId}">
												${fn:substring(row.newsTittle,0,30)}...
											</a>
										</td>
									</c:if>
									<c:if test="${fn:length(row.newsTittle) <= 30 }">
					...
					<div class="dataTables_paginate paging_bs_full" id="datatable1_paginate" style="height:50px">
						...
						<ul class="pagination" style="padding-left: 0;margin: 20px 0;display: inline-block;">
							<li ${pager.pageNum==1?'class="disabled"':''}><a
								href="AstockNews?pageNum=1">首页</a></li>
							<li ${pager.pageNum==1?'class="disabled"':''}><a
								href="AstockNews?pageNum=${pager.pageNum-1 }">上页</a></li>
							<li ${pager.pageNum==pager.pageCount?'class="disabled"':''}><a
								href="AstockNews?pageNum=${pager.pageNum+1 }">下页</a></li>
							<li ${pager.pageNum==pager.pageCount?'class="disabled"':''}><a
								href="AstockNews?pageNum=${pager.pageCount }">末页</a></li>
							<li><a>转到 </a></li>
