...
oSelectList.onclick = function(e, a) {
	...
	var bRadio = document.getElementsByName('radio3');
	var cRadio = document.getElementsByName('radio4');

    if( isCusPrice ) {
      radioVal = oCustext1.value + '-' + oCustext2.value + '分';
    } else {
      radioVal = '';
    }
    ...
    if (tagName == 'A') { //如果点击 的是 A
        ...
        if (!oPrevInput) { //如果上一个节点没有则认为点击的是 '不限'
			...
			//1.没考虑：
            if (trim(prev(parent).innerHTML) == '综合得分') { //这里是直接根据 text来比较的.建议加个自定义属性作标识符
                for (var i = 0; i < aRadio.length; i++) {
                    aRadio[i].checked = false;
                }
                isCusPrice = false;
			...
			if (trim(prev(parent).innerHTML) == '技术水平') { //这里是直接根据 text来比较的.建议加个自定义属性作标识符
                for (var i = 0; i < bRadio.length; i++) {
                    bRadio[i].checked = false;
                }
                isCusPrice = false;
			...
			if (trim(prev(parent).innerHTML) == '风险指数') { //这里是直接根据 text来比较的.建议加个自定义属性作标识符
				for (var i = 0; i < cRadio.length; i++) {
                    cRadio[i].checked = false;
                }
                isCusPrice = false;
    ...
    for (var i = 0; i < aRadio.length; i++) {
        if (aRadio[i].checked) {
            ...
            isCusPrice = false;
			if (radioVal) {
			   infor += '<div class=\"selectedInfor selectedShow\"><span>综合得分</span><label>' + radioVal + '</label><em p="2"></em></div>';
		    }
            break;
        }
    }
	for (var i = 0; i < bRadio.length; i++) {
        if (bRadio[i].checked) {
            radioVal = next(bRadio[i]).innerHTML;
            isCusPrice = false;
			if (radioVal) {
			   infor += '<div class=\"selectedInfor selectedShow\"><span>技术水平</span><label>' + radioVal + '</label><em p="3"></em></div>';
		    }
            break;
        }
    }
	for (var i = 0; i < cRadio.length; i++) {
        if (cRadio[i].checked) {
            radioVal = next(cRadio[i]).innerHTML;
            isCusPrice = false;
			if (radioVal) {
			   infor += '<div class=\"selectedInfor selectedShow\"><span>风险指数</span><label>' + radioVal + '</label><em p="4"></em></div>';
		    }
            break;
        }
    }
...
$('.clearList').find('em').live('click',function(){
    ...
    for(var i = 0, len = aItemTxt.length; i < len; i++) {
		 ...
		 console.log(aItemTxt[i].innerHTML+'---'+html);
