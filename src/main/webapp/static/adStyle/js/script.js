var App = function() {
	...
	var handleSidebar = function() {
		...
		jQuery('.sidebar-menu .has-sub .sub .has-sub-sub > a').click(
				function() {
	...
	var collapseSidebar = function() {
		var iconElem = document.getElementById("sidebar-collapse")
	...
	var responsiveSidebar = function() {
		...
		if (width < 768) {
			...
			var menu = $('.sidebar');
			if (menu.parent('.slimScrollDiv').size() === 1) { // destroy
																// existing
																// instance
																// before
																// resizing
				...
				.click(
						function() {
							...
							if (is_mobile && !(is_mini_menu)) {
								...
								var iconElem = document.getElementById(
										"sidebar-collapse").querySelector(
										'[class*="fa-"]');
								var iconLeft = iconElem
										.getAttribute("data-icon1");
								var iconRight = iconElem
										.getAttribute("data-icon2");
								// If sidebar is collapsed
								if (collapsed) {
									/* For Navbar */
									...
									jQuery('#main-content').removeClass(
											"margin-left-50");
									jQuery('.sidebar-collapse i').removeClass(
											iconRight);
									jQuery('.sidebar-collapse i').addClass(
											iconLeft);
		...
		$("body")
				.click(
						function(z) {
							...
							if ($.inArray("team-status", y) == -1
									&& $.inArray("team-status-toggle", y) == -1
									&& $(z.target).parents().index(
											$(".team-status")) == -1
									&& $(z.target).parents(
											".team-status-toggle").length == 0) {
								w()
							}
						});
	...
	var handleBoxTools = function() {
		...
		/* Close */
		jQuery('.box .tools a.remove').click(
				function() {
					var removable = jQuery(this).parents(".box");
					if (removable.next().hasClass('box')
							|| removable.prev().hasClass('box')) {
	...
	var handleMagicSuggest = function() {
		var jsonData = [];
		var cities = 'New York,Los Angeles,Chicago,Houston,Paris,Marseille,Toulouse,Lyon,Bordeaux,Philadelphia,Phoenix,San Antonio,San Diego,Dallas,San Jose,Jacksonville'
				.split(',');
		...
		$('.radio1').on(
				...
				function() {
					$('.radio1').bootstrapSwitch(
							'toggleRadioStateAllowUncheck', false);
				});
	}
	/*-----------------------------------------------------------------------------------*/
	/*
				...
				.ready(
						function() {
							jQuery.fn.anim_progressbar = function(aOptions) {
								...
								var aDefOpts = {
									start : new Date(), // now
									finish : new Date().setTime(new Date()
											.getTime()
											+ 60 * iCms), // now + 60 sec
									interval : 100
								}
								...
								return this
										.each(function() {
											...
											var vInterval = setInterval(
													function() {
														var iLeftMs = aOpts.finish
																- new Date(); // left
																				// time
																				// in
																				// MS
														var iElapsedMs = new Date()
																- aOpts.start, // elapsed
																				// time
																				// in
																				// MS
														iDays = parseInt(iLeftMs
																/ iDms), // elapsed
																			// days
														iHours = parseInt((iLeftMs - (iDays * iDms))
																/ iHms), // elapsed
																			// hours
														iMin = parseInt((iLeftMs
																- (iDays * iDms) - (iHours * iHms))
																/ iMms), // elapsed
																			// minutes
														iSec = parseInt((iLeftMs
																- (iDays * iDms)
							...
							var iNow = new Date()
									.setTime(new Date().getTime() + 5 * 1000); // now
																				// plus
																				// 5
																				// secs
							var iEnd = new Date()
									.setTime(new Date().getTime() + 15 * 1000); // now
																				// plus
																				// 15
																				// secs
				...
				.select2(
						{
							...
							ajax : { // instead of writing the function to
										// execute the request we use Select2's
										// convenient helper
								...
								data : function(term, page) {
									return {
										q : term, // search term
										page_limit : 10,
										apikey : "uekzdmffsrmqzwdtcgmc5yu9" // please
																			// do
																			// not
																			// copy
																			// API.
								...
								results : function(data, page) { // parse the
									...
									// functions we do not need to alter remote
									// JSON data
									return {
										results : data.movies
									};
							...
							initSelection : function(element, callback) {
								// the input tag has a value attribute preloaded
								// that points to a preselected movie's id
								// this function resolves that id attribute to
								// an object that select2 can render
								// using its formatResult renderer - that way
								// the movie name is shown preselected
								var id = $(element).val();
								if (id !== "") {
									$
											.ajax(
							...
							formatResult : movieFormatResult, // omitted for
																// brevity, see
																// the source of
																// this page
							formatSelection : movieFormatSelection, // omitted
																	// for
																	// brevity,
																	// see the
																	// source of
	...
	var handleDropzone = function() {
		try {
			$(".dropzone")
					.dropzone(
							{
								paramName : "file", // The name that will be
	...
	var handleXcharts = function() {
		...
		function chart1() {
			...
			var order = [ 0, 1, 0, 2 ], i = 0, xFormat = d3.time.format('%A'), chart = new xChart(
					'line-dotted', data[order[i]], '#chart1', {
						axisPaddingTop : 5,
		...
		function chart3() {
			var tt = document.createElement('div'), leftOffset = -(~~$('html')
					.css('padding-left').replace('px', '') + ~~$('body').css(
		...
		function chart6() {
			var errorBar = {
				enter : function(self, storage, className, data, callbacks) {
					var insertionPoint = xChart.visutils.getInsertionPoint(9), container,
					// Map each error bar into 3 points, so it's easier to draw
					// as a single path
					// Converts each point to a triplet with y from (y - e) to
					// (y + e)
					// It would be better to use the `preUpdateScale` method
					// here,
					...
					// The insertionPoint is a special method that helps us
					// insert this
					// vis at a particular z-index
					// In this case, we've chosen the highest point (above
					// everything else)
					container.enter().insert('g', insertionPoint).attr(
							'class',
							function(d, i) {
								return 'errorLine'
										+ className.replace(/\./g, ' ')
					...
					paths.enter().insert('path').style('opacity', 0).attr(
							'd',
							d3.svg.line().x(
									function(d) {
										// We offset by half the rangeBand,
										// because this is a bar chart
										return self.xScale(d.x)
												+ self.xScale.rangeBand() / 2;
									}).y(function(d) {
				...
				update : function(self, storage, timing) {
					// This is mostly duplication to the d3.svg.line from the
					// enter() method
					storage.paths.transition().duration(timing).style(
							'opacity', 1).attr(
							'd',
							d3.svg.line().x(
									function(d) {
				...
				exit : function(self, storage, timing) {
					storage.paths.exit().transition().duration(timing).style(
							'opacity', 0);
				},
	...
	var handleGage = function() {
		...
		window.onload = function() {
			...
			var g3 = new JustGage({
				...
				title : "Custom Colors",
				label : "",
				levelColors : [ Theme.colors.red, Theme.colors.yellow,
						Theme.colors.green ]
			});
		...
		var calendar = $('#calendar').fullCalendar(
				{
					...
					editable : true,
					editable : true,
					droppable : true, // this allows things to be dropped onto
										// the calendar !!!
	...
	var handleHover = function() {
		$('.filter-content').hover(
				function() {
					...
					hoverContent.removeClass('fadeOut').addClass(
	...
	var handleColorbox = function() {
		...
		function resizeColorBox() {
			...
			resizeTimer = setTimeout(function() {
				...
				if (jQuery('#cboxOverlay').is(':visible')) {
					$.colorbox.resize({
						width : ($(window).width() > (myWidth + 20)) ? myWidth
		...
		// Resize Colorbox when resizing window or changing mobile device
		// orientation
	...
	var handleBackstretch = function() {
		$.backstretch([ "img/login/1.jpg", "img/login/2.jpg",
				"img/login/3.jpg", "img/login/4.jpg" ], {
			duration : 3000,
			fade : 750
		});
	}
	...
	var handleChat = function(elem) {
		var append = function() {
			...
			msg += '<h4 class="media-heading">You <span class="pull-left"><abbr id="curr-time" class="timeago" title="'
					+ curr_time
					+ '" >'
					+ curr_time
					+ '</abbr> <i class="fa fa-clock-o"></i></span></h4>';
			msg += text;
			msg += '</div>';
			msg += '</li>';
	...
	var handleSliderNav = function() {
		...
		$('#address-book .slider-content ul li ul li a').click(
				function(e) {
					e.preventDefault();
					...
					// Set the image
					$('#contact-card .headshot img').attr('src',
							'img/addressbook/' + img_id + '.jpg');
					contact_card.removeClass('animated fadeInUp').addClass(
							'animated fadeInUp');
	...
	var handleDashFlotCharts = function() {
		function chartMonth() {
			var data1 = [ [ 0, 1.5 ], [ 1, 2 ], [ 2, 1 ], [ 3, 1.5 ],
					[ 4, 2.5 ], [ 5, 2 ], [ 6, 2 ], [ 7, 0.5 ], [ 8, 1 ],
					[ 9, 1.5 ], [ 10, 2 ], [ 11, 2.5 ], [ 12, 2 ], [ 13, 1.5 ],
					[ 14, 2.8 ], [ 15, 2 ], [ 16, 3 ], [ 17, 2 ], [ 18, 2.5 ],
					[ 19, 3 ], [ 20, 2.5 ], [ 21, 2 ], [ 22, 1.5 ],
					[ 23, 2.5 ], [ 24, 2 ], [ 25, 1.5 ], [ 26, 1 ],
					[ 27, 0.5 ], [ 28, 1 ], [ 29, 1 ], [ 30, 1.5 ], [ 31, 1 ] ];
			var data2 = [ [ 0, 2.5 ], [ 1, 3.5 ], [ 2, 2 ], [ 3, 3 ], [ 4, 4 ],
					[ 5, 3.5 ], [ 6, 3.5 ], [ 7, 1 ], [ 8, 2 ], [ 9, 3 ],
					[ 10, 4 ], [ 11, 5 ], [ 12, 4 ], [ 13, 3 ], [ 14, 5 ],
					[ 15, 3.5 ], [ 16, 5 ], [ 17, 4 ], [ 18, 5 ], [ 19, 6 ],
					[ 20, 5 ], [ 21, 4 ], [ 22, 3 ], [ 23, 5 ], [ 24, 4 ],
					[ 25, 3 ], [ 26, 2 ], [ 27, 1 ], [ 28, 2 ], [ 29, 2 ],
					[ 30, 3 ], [ 31, 2 ] ];
			...
			$("#placeholder").bind(
					...
					function(event, ranges) {
						...
						plot = $.plot("#placeholder", getData(
								ranges.xaxis.from, ranges.xaxis.to), $.extend(
								true, {}, options, {
									xaxis : {
	...
	var handleGritter = function() {
		...
		setTimeout(function() {
			...
			var unique_id = $.gritter.add({
				...
				// (bool | optional) if you want it to fade out on its own or
				// just sit there
				sticky : true,
	...
	return {
		...
		init : function() {
			...
			if (App.isPage("index")) {
				handleDateTimePickers(); // Function to display Date
											// Timepicker
				handleSparkline(); // Function to display Sparkline charts
				handleDashFlotCharts(); // Function to display flot charts in
										// dashboard
			...
			if (App.isPage("elements")) {
				...
				handleDateColorpicker(); // Function to handle date and time
											// picker
				handleRaty(); // To show star ratings
				handleTimeAgo(); // Function to handle timestamps
			}
			if (App.isPage("button_icons")) {
				handleStatefulButtons(); // Function to display stateful
											// buttons
			...
			checkLayout(); // Function to check if mini menu/fixed header is
							// activated
			handleSidebar(); // Function to display the sidebar
			handleSidebarCollapse(); // Function to hide or show sidebar
			handleSidebarAndContentHeight(); // Function to hide sidebar and
												// main content height
			...
			handleNavbarFixedTop(); // Function to check & handle if navbar is
									// fixed top
